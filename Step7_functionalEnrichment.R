...
mydata.df.sub <- dplyr::select(myTopHits.df, geneID)
# construct a named vector
mydata.gsea <- myTopHits$LogFC
names(mydata.gsea) <- as.character(myTopHits$LogFC)
...
library(msigdbr) # access to msigdb collections directly within R
library(enrichplot) # great for making the standard GSEA enrichment plots
gost.res_up <- gost(rownames(module_up), organism = "hsapiens", correction_method = "fdr")
gostplot(gost.res_up, interactive = T, capped = T) #set interactive=FALSE to get plot for publications
gost.res_down <- gost(rownames(module_down), organism = "hsapiens", correction_method = "fdr")
...
# grab the dataframe you made in step3 script
# Pull out just the columns corresponding to gene symbols and LogFC for at least one pairwise comparison for the enrichment analysis
mydata.df.sub <- dplyr::select(moduleData.df, geneSymbol)
mydata.gsea <- moduleData.df
names(mydata.gsea) <- as.character(moduleData.df$geneID)
mydata.gsea <- sort(mydata.gsea, decreasing = TRUE)

# run GSEA using the 'GSEA' function from clusterProfiler
myGSEA.res <- GSEA(myTopHits.df, TERM2GENE=hs_gsea_c2, verbose=FALSE)
myGSEA.df <- as_tibble(myGSEA.res@result)
