...
1# Make a DGElist from your counts, and plot ----
...
colnames(log2.cpm.df) <- c("geneID", sampleLabels)
log2.cpm.df.pivot <- pivot_longer(log2.cpm.df, # dataframe to be pivoted
                                  cols = Sample1:Sample98, # column names to be stored as a SINGLE variable
                                  names_to = "samples", # name of that new variable (column)
                                  values_to = "expression") # name of new variable (column) storing all the values (data)
...
log2.cpm.filtered.df <- as_tibble(log2.cpm.filtered, rownames = "geneID")
colnames(log2.cpm.filtered.df) <- c("geneID", sampleLabels)
log2.cpm.df.pivot <- pivot_longer(log2.cpm.filtered.df, # dataframe to be pivoted
                                           cols = M1h_LEPZQ_1:X24h_NMRI_3, # column names to be stored as a SINGLE variable
                                           names_to = "samples", # name of that new variable (column)
                                           values_to = "expression") # name of new variable (column) storing all the values (data)
...
myDGEList.filtered <- calcNormFactors(myDGEList.filtered, method = "TMM")
log2.cpm.filtered <- cpm(myDGEList.filtered.norm, log=TRUE)
log2.cpm.filtered.df <- as_tibble(log2.cpm.filtered.norm, rownames = "geneID")
colnames(log2.cpm.filtered.df) <- c("geneID", sampleLabels)
log2.cpm.df.pivot <- pivot_longer(log2.cpm.filtered.df, # dataframe to be pivoted
                                                cols = M1h_LEPZQ_1:X24h_NMRI_3, # column names to be stored as a SINGLE variable
                                                names_to = "samples", # name of that new variable (column)
