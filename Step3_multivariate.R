...
distance <- dist(t(log2.cpm.filtered), method = "maximum") #other distance methods are "euclidean", maximum", "manhattan", "canberra", "binary" or "minkowski"
...
pca.res <- prcomp(t(log2.cpm.filtered), scale.=F, retx=T)
...
pca.res <- prcomp(t(log2.cpm.filtered.df), scale.=F, retx=T)
pc.var <- pca.res$sdev^2 # sdev^2 captures these eigenvalues from the PCA result
...
mydata.df <- mutate(log2.cpm.filtered.df,
                    F24h.AVG = (F24_LE_1 + F24_LE_2 + F24_LE_3/3, 
                    FCtl.AVG = (FCtl_LE_1 + FCtl_LE_2 + FCtl_LE_3/3,
                    #now make columns comparing each of the averages above that you're interested in
                    LogFC = (F24h.AVG - FCtl.AVG)) %>% #note that this is the first time you've seen the 'pipe' operator
  mutate_if(is.numeric, round, 2)
